function [U_ B_ V_ RMSE_tr RMSE_te MAE_tr MAE_te ira] = CST(train_vec, test_vec, U0, V0, tradeoff_lambda)
...
Recall  = zeros(MAX_EPOCH, 1);
Precision  = zeros(MAX_EPOCH, 1);
...
% --- Evaluation
i=1;
[rmse, mae, ira] = EvalPred(U,V,B,train_vec,1,0); RMSE_tr(i) = rmse; 
[rmse, mae, ira] = EvalPred(U,V,B,test_vec,1,0);  RMSE_te(i) = rmse; MAE_te(i) = mae; 
...
% step 3: Estimate B
    ...
    % --- Check Performance
    [rmse, mae,ira] = EvalPred(U,V,B,train_vec,1,0); RMSE_tr(i) = rmse; MAE_tr(i) = mae; 
    [rmse, mae,ira] = EvalPred(U,V,B,test_vec,1,0);  RMSE_te(i) = rmse; MAE_te(i) = mae; 
    fprintf( 1, 'epoch: %d, tr: %6.4f(RMSE), %6.4f(MAE); te: %6.4f(RMSE), %6.4f(MAE)\n', i, RMSE_tr(i), MAE_tr(i),  RMSE_te(i), MAE_te(i) );
    
    fprintf( 1, 'epoch: %d, %6.4f(Recall), %6.4f(Precision)\n', i, Recall(i), Precision(i) );
